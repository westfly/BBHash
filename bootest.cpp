...
class bfile_iterator
    : public std::iterator<std::forward_iterator_tag, u_int64_t> {
public:
    ...
    friend bool operator==(bfile_iterator const& lhs,
                           bfile_iterator const& rhs) {
        if (!lhs._is || !rhs._is) {
            if (!lhs._is && !rhs._is) {
                return true;
            }
...
public:
    ...
    bfile_iterator begin() const { return bfile_iterator(_is); }
...
// simple iterator to generate list of distinct u_int64_t keys (not random, just
// equally distributed in [0;ULLONG_MAX])
class uint64_iterator
    : public std::iterator<std::forward_iterator_tag, u_int64_t> {
public:
    uint64_iterator()
        : _nb_elem(0), _curr(ULLONG_MAX), _step(0), _nb_iterated(0) {}
    ...
    uint64_iterator(u_int64_t nb_elem, u_int64_t stop)
        : _nb_elem(nb_elem), _curr(0), _nb_iterated(0), _stop(stop) {
    ...
    uint64_iterator& operator++() {
        ...
        _curr = _curr + _step;
        _nb_iterated++;
        if (_nb_iterated >= _stop)
            _nb_elem = 0;
        return *this;
    }

    friend bool operator==(uint64_iterator const& lhs,
                           uint64_iterator const& rhs) {
        if (!lhs._nb_elem || !rhs._nb_elem) {
            if (!lhs._nb_elem && !rhs._nb_elem) {
                return true;
            }
            else {
...
class uint64_range {
public:
    uint64_range(u_int64_t nb_elem, u_int64_t nbiter)
        : _nb_elem(nb_elem), _stop(nbiter) {}
    ~uint64_range() {}

    uint64_iterator begin() const { return uint64_iterator(_nb_elem, _stop); }
    uint64_iterator end() const { return uint64_iterator(); }
...
struct stats_accumulator {
    stats_accumulator() : m_n(0), m_mean(0), m_m2(0) {}
    ...
    double variance() const { return m_m2 / (m_n - 1); }
...
void multipleMPHF(const vector<vector<u_int64_t>>& datas,
                  uint                             start,
                  uint                             n,
                  uint                             bucketNum) {
    for (uint ii(start); ii < start + n; ++ii) {
        auto data_iterator2 = boomphf::range(
            static_cast<const u_int64_t*>(&datas[ii][0]),
            static_cast<const u_int64_t*>(&datas[ii][0] + datas[ii].size()));
        MPHFs[bucketNum * nMphfByBucket + ii] =
            boomphf::mphf<u_int64_t, hasher_t>(datas[ii].size(),
                                               data_iterator2,
                                               1,
                                               gammaFactor,
                                               write_each,
...
void compactBucket(uint start, uint n) {
    ...
    for (uint i(start); i < start + n; ++i) {
        ...
        for (uint tn(0); tn < 1; ++tn) {
            threads.push_back(thread(multipleMPHF,
                                     datas,
                                     tn * (nMphfByBucket / 1),
                                     nMphfByBucket / 1,
                                     i));
...
void bench_mphf_lookup(phf_t* bphf, Range const& input_range) {
    ...
    printf("BBhash bench lookups average %.2f ns +- stddev  %.2f %%   "
           "(fingerprint %llu)  \n",
           1000.0 * stats.mean(),
           stats.relative_stddev(),
           dumb);
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        ...
        printf("\t-buckets\n");
        printf(
            "\t-outquery (bench the fp rate of the mphf)\n"); // bench fp rate
        printf("\t-onthefly (generates key on the fly without storing them on "
               "disk or in ram)\n");
    ...
    for (int ii = 4; ii < argc; ii++) {
        if (!strcmp("-check", argv[ii]))
            check_correctness = true;
        if (!strcmp("-bench", argv[ii]))
            bench_lookup = true;
        if (!strcmp("-save", argv[ii]))
            save_mphf = true;
        if (!strcmp("-load", argv[ii]))
            load_mphf = true;
        if (!strcmp("-inram", argv[ii]))
            from_disk = false;
        if (!strcmp("-buckets", argv[ii]))
            buckets = true;
        if (!strcmp("-outquery", argv[ii]))
            bench_lookup_out = true;
        if (!strcmp("-onthefly", argv[ii]))
            on_the_fly = true;
        if (!strcmp("-nodisk", argv[ii]))
            write_each = false;
    }
    ...
    if (!from_disk && !buckets) {
        ...
        if (!on_the_fly) {
            ...
            if (bench_lookup) {
                ...
                // create a test file
                // if n < 10 M take all elements, otherwise regular sample to
                // have 10 M elements
    ...
    if (buckets) {
        ...
        for (auto const& key : data_iterator) {
            u_int64_t hash =
                (korenXor(key) % (nBuckets * nMphfByBucket) / nMphfByBucket);

            if (buffers[hash].size() == buffsize) {
                fwrite(buffers[hash].data(),
                       sizeof(u_int64_t),
                       buffers[hash].size(),
                       vFiles[hash]);
        ...
        for (int ii = 1; ii < nBuckets * nMphfByBucket; ii++) {
            nb_elem_in_previous_buckets[ii] =
                nb_elem_in_previous_buckets[ii - 1] + elinbuckets[ii - 1];
        }
        ...
        for (uint n(0); n < nthreads; ++n) {
            threads.push_back(thread(
                compactBucket, n * nBuckets / nthreads, nBuckets / nthreads));
        }
        for (auto& t : threads) {
            t.join();
        }
        ...
        if (check_correctness) {
            ...
            for (u_int64_t i = 0; i < nelem; i++) {
                ...
                u_int64_t mphf_value = MPHFs[hash].lookup(current2) +
                                       nb_elem_in_previous_buckets[hash];
                if (mphf_value >= nelem) {
                    range_problems++;
            ...
            // printf("BooPHF %llu lookups in  %.2fs,  approx  %.2f ns per
            // lookup \n", nelem, (double)(end - begin) / CLOCKS_PER_SEC,
            // ((double)(end - begin) / CLOCKS_PER_SEC)*1000000000/nelem);
        }
        ...
        if (bench_lookup) {
            ...
            for (size_t run = 0; run < runs; ++run) {
                for (size_t ii = 0; ii < sample.size(); ++ii) {
                    ...
                    uint64_t hash =
                        korenXor(sample[ii]) % (nBuckets * nMphfByBucket);
                    mphf_value = MPHFs[hash].lookup(sample[ii]) +
                                 nb_elem_in_previous_buckets[hash];
                    dumb += mphf_value;
            ...
            printf("BBhash buckets bench lookups average %.2f ns +- stddev  "
                   "%.2f %%   (fingerprint %llu)  \n",
                   1000.0 * stats.mean(),
                   stats.relative_stddev(),
                   dumb);
    ...
    if (!load_mphf) {
        ...
        if (on_the_fly) {
            ...
            bphf = new boomphf::mphf<u_int64_t, hasher_t>(
                nelem, data_iterator, nthreads, gammaFactor, write_each);
        }
        else if (from_disk) {
            auto data_iterator = file_binary("keyfile");
            bphf = new boomphf::mphf<u_int64_t, hasher_t>(
            ...
            auto data_iterator =
                boomphf::range(static_cast<const u_int64_t*>(data),
                               static_cast<const u_int64_t*>(data + nelem));
            bphf = new boomphf::mphf<u_int64_t, hasher_t>(
                nelem, data_iterator, nthreads, gammaFactor, write_each);
        }
    ...
    if (bench_lookup_out) {
        ...
        printf("query %i elem  out of set  FP rate %.2f   nb issues %llu    "
               "lookup %.2f  ns \n",
               nrandom,
               nb_fp / (float)nrandom,
               nb_out_of_range,
