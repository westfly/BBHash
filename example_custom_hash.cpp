...
public:
    ...
    // BBhash will use the 'seed' paramater to generate two different hash
    // values form this key.
    // then it will generate internally a sequence of hash values using
    // xorshifts, using these two first hash values as starting point.
...
int main(int argc, char* argv[]) {
    ...
    // mphf takes as input a c++ range. A simple array of keys can be wrapped
    // with boomphf::range but could be from a user defined iterator (enabling
    // keys to be read from a file or from some complex non-contiguous
    // structure)
    auto data_iterator =
    ...
    double gammaFactor =
        2.0; // lowest bit/elem is achieved with gamma=1, higher values lead to
             // larger mphf but faster construction/query
    // gamma = 2 is a good tradeoff (leads to approx 3.7 bits/key )

    // build the mphf
    bphf = new boomphf::mphf<u_int64_t, Custom_uint64_Hasher>(
        nelem, data_iterator, nthreads, gammaFactor);
