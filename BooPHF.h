...
public:
    ...
    bfile_iterator<type_elem> end() const {
...
private:
    ...
    void generate_hash_seed() {
        static const uint64_t rbase[MAXNBFUNC] = {0xAAAAAAAA55555555ULL,
        ...
        for (size_t i = 0; i < MAXNBFUNC; ++i) {
            _seed_tab[i] =
                _seed_tab[i] * _seed_tab[(i + 3) % MAXNBFUNC] + _user_seed;
        }
    }
...
public:
    ...
    hash_set_t operator()(const Item& key) {
        ...
        for (size_t ii = 2;
             ii < 10 /* it's much better have a constant here, for inlining;
                        this loop is super performance critical*/
             ;
             ii++) {
            hset[ii] = next(s);
        }
...
public:
    ...
    // clear collisions in interval, only works with start and size multiple of
    // 64
...
struct thread_args {
    ...
    std::shared_ptr<void>
        it_p; /* used to be "Iterator it" but because of fastmode, iterator is
                 polymorphic; TODO: think about whether it should be a
                 unique_ptr actually */
...
public:
    ...
    mphf(size_t       n,
         Range const& input_range,
         int          num_thread = 1,
         double       gamma = 2.0,
         bool         writeEach = true,
         bool         progress = true,
         float        perc_elem_loaded = 0.03)
        : _gamma(gamma), _hash_domain(size_t(ceil(double(n) * gamma))),
          _nelem(n), _num_thread(num_thread),
        ...
        if (_withprogress) {
            ...
            printf("for info, total work write each  : %.3f    total work "
                   "inram from level %i : %.3f  total work raw : %.3f \n",
                   total_writeEach,
                   _fastModeLevel,
                   total_fastmode_ram,
                   total_raw);
    ...
    uint64_t lookup(elem_t elem) {
        ...
        if (level == (_nb_levels - 1)) {
            ...
            // non_minimal_hp =  level_hash %  _levels[level].hash_domain; // in
            // fact non minimal hp would be  + _levels[level]->idx_begin
            non_minimal_hp =
    ...
    uint64_t totalBitSize() {
        ...
        uint64_t totalsize =
            totalsizeBitset + _final_hash.size() * 42 *
                                  8; // unordered map takes approx 42B per elem
                                     // [personal test] (42B with uint64_t key,
                                     // would be larger for other type of elem)

        printf("Bitarray    %12llu  bits (%.2f %%)   (array + ranks )\n",
               totalsizeBitset,
               100 * (float)totalsizeBitset / totalsize);
        printf("Last level hash  %12lu  bits (%.2f %%) (nb in last level hash "
               "%lu)\n",
    ...
    void pthread_processLevel(std::vector<elem_t>&      buffer,
            ...
            for (uint64_t ii = 0; ii < inbuff; ii++) {
                ...
                if (level == i) // insert into lvl i
                {
                    ...
                    if (_fastmode && i == _fastModeLevel) {
                        ...
                        // si depasse taille attendue pour setLevelFastmode,
                        // fall back sur slow mode mais devrait pas arriver si
                        // hash ok et proba avec nous
                    ...
                    // insert to level i+1 : either next level of the cascade or
                    // final hash if last level reached
                    if (i == _nb_levels -
                                 1) // stop cascade here, insert into exact hash
                    {
                        ...
                        pthread_mutex_lock(
                            &_mutex); // see later if possible to avoid this,
                                      // mais pas bcp item vont la
                        // calc rank de fin  precedent level qq part, puis init
                        // hashidx avec ce rank, direct minimal, pas besoin
                        // inser ds bitset et rank
                        ...
                        // insert elem into curr level on disk --> sera utilise
                        // au level+1 , (mais encore besoin filtre)
        ...
        if (_writeEachLevel && writebuff > 0) {
            ...
            flockfile(_currlevelFile);
            fwrite(
                myWriteBuff.data(), sizeof(elem_t), writebuff, _currlevelFile);
            funlockfile(_currlevelFile);
            writebuff = 0;
        }
    }
...
private:
    void setup() {
        ...
        for (int ii = 0; ii < _nb_levels; ii++) {
            ...
            // round size to nearest superior multiple of 64, makes it easier to
            // clear a level
            _levels[ii].hash_domain =
                (((uint64_t)(_hash_domain * pow(_proba_collision, ii)) + 63) /
                 64) *
    ...
    uint64_t getLevel(hash_pair_t& bbhash,
                      elem_t       val,
                      int*         res_level,
                      int          maxlevel = 100,
                      int          minlevel = 0)
    ...
    void processLevel(Range const& input_range, int i) {
        ...
        if (_writeEachLevel && (i > 1)) {
            ...
            t_arg.it_p = std::static_pointer_cast<void>(
                std::make_shared<disklevel_it_type>(
                    data_iterator_level.begin()));
            t_arg.until_p = std::static_pointer_cast<void>(
            ...
            for (int ii = 0; ii < _num_thread; ii++)
                pthread_create(&tab_threads[ii],
                               NULL,
                               thread_processLevel<elem_t,
                                                   Hasher_t,
                                                   Range,
                                                   disklevel_it_type>,
                               &t_arg); //&t_arg[ii]

            // must join here before the block is closed and file_binary is
            // destroyed (and closes the file)
            for (int ii = 0; ii < _num_thread; ii++) {
            ...
            if (_fastmode && i >= (_fastModeLevel + 1)) {
                ...
                /* we'd like to do t_arg.it = data_iterator.begin() but types
                 are different; so, casting to (void*) because of that; and we
                 remember the type in the template */
                typedef decltype(setLevelFastmode.begin()) fastmode_it_type;
                t_arg.it_p = std::static_pointer_cast<void>(
                    std::make_shared<fastmode_it_type>(
...
private:
    ...
    // fast build mode , requires  that _percent_elem_loaded_for_fastMode %
    // elems are loaded in ram
